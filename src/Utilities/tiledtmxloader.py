...
__version__ = u'$Id: tiledtmxloader.py 228 2009-07-23 20:28:46Z dr0iddr0id $'
...
class ImageLoaderPygame(IImageLoader):
    ...
    def load_image(self, filename, colorkey=None):
        ...
        if img is None:
            ...
            # CAD ADDITION - Only do the the convert once on load.
            if (img.get_alpha()):
    ...
    def convert(self):
        ...
        for tile_set in self.tile_sets:
            ...
            for img in tile_set.images:
                # CAD FIX - make sure it's not a tuple already
                if img.trans and (type(img.trans) != type( () )):
        ...
        for obj_group in self.object_groups:
            ...
            # CAD FIX - object groups have a visible property too!
            obj_group.visible = bool(int(obj_group.visible))
            ...
            for tile in tile_set.tiles:
                for img in tile.images:
                    if not img.content and not img.source:
                        ...
                        # CAD - Add tile into the dict
                        self.indexed_tiles[int(tile_set.firstgid) + int(tile.id)] = (0, 0, indexed_img, tile)
                    else:
                        if img.source:
                            self._load_image_from_source(tile_set, img)
                        else:
...
class TileLayer(object):
    ...
    :Ivariables:
        ...
        order : int
            the index in the layer stack - CAD ADDITION
...
class TileMapParser(object):
    ...
    """
    #CAD ADDITION - New variable to preserve layer ordering in the map.
    #        ObjectGroups should really behave like layers.
    def __init__(self):
        self._layersInWorld = 0 
    ...
    def _build_layer(self, layer_node, world_map):
        layer = TileLayer()
        layer.order = self._layersInWorld   #CAD
        ...
        self._get_properties(layer_node, layer) # CAD make properties work on layers
    ...
    def _build_world_map(self, world_node):
        ...
        # changed to preserve layer ordering - CAD
        """
        ...
        for node in self._get_nodes_list(world_node.childNodes, [u'layer', u'objectgroup']):
            if node.nodeName == u'layer':
                self._build_layer(node, world_map)
            elif node.nodeName == u'objectgroup':
                self._build_object_groups(node, world_map)
            self._layersInWorld += 1
            
        return world_map

    def _build_object_groups(self, object_group_node, world_map):
        object_group = MapObjectGroup()
        object_group.order = self._layersInWorld    #CAD
    ...
    # CAD ADDITION -  to preserve layer order, we can get nodes of various names at the same time.
    def _get_nodes_list(self, nodes, names):
        for node in nodes:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    cam_offset_y -= moveAmount
                elif event.key == pygame.K_UP:
                    cam_offset_y += moveAmount
                elif event.key == pygame.K_LEFT:
                    cam_offset_x += moveAmount
        ...
        for y in xrange(layer.pixel_height - world_map.tileheight,
            ...
            for x in xrange(0, layer.pixel_width, world_map.tilewidth):
                ...
                if image_id:
                    ...
                    sprites.append(pyglet.image.Sprite(image_file,
                        x, y , batch=batch,
